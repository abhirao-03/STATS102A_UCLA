...
#### Test Case
Here I showcase the student with UID 123456797 and the imputation of their `NA` scores.
```{r test-case}
gradebook[8,]
...
To convert the `gradebook` dataset into tidy data we use the `pivot_longer()` function

```{r tidy time}
gradebook <- gradebook %>% pivot_longer(cols = !UID,
